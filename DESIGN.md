...
| `metadata` | jsonb | Extracted preview metadata (`name`, `epoch`, `version`, `release`, `arch`, `summary`, `description`, `url`, `license`, `rpm_sourcerpm`, `rpm_group`, `rpm_vendor`, `rpm_buildhost`, `size_installed`, `build_time`, dependencies, files, and changelogs) using the same dependency, file, and changelog shapes as package detail responses |
