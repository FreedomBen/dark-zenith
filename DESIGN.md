...
| `repositories_preparation_complete` | boolean | Durable end-of-scan marker for repository preparation (default `false` for user-wide transitions) |
| `packages_preparation_complete` | boolean | Durable end-of-scan marker for package preparation (default `false` for user-wide transitions) |
...
| `phase_next_attempt_at` | timestamp | Earliest time the next phase batch may run; null while active item work or terminal |
...
| `PREVIOUS_SECRET_KEY_BASE` | — | Optional. Prior value of `SECRET_KEY_BASE` during a rotation window so current and prepared encrypted GPG private keys can be decrypted while background jobs migrate them. Remove once every ciphertext has been migrated. |
