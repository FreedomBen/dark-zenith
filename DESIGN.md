...
| `slug` | string | Normalized lowercase URL-safe identifier (e.g., `stable`, `nightly`); must match `^[a-z0-9][a-z0-9_-]{0,63}$`; the value `new` is reserved for the repository-creation route and rejected; slugs of deleted repositories are retired and unavailable to other users (see Retired Slugs) |
...
When a repository is deleted, its slug is retired rather than released, so another user cannot recreate a repository at the same URL and serve packages to clients whose `.repo` files still point at it.

| Field | Type | Description |
|---|---|---|
| `id` | UUID | Primary key |
| `slug` | string | The retired slug |
| `user_id` | UUID | FK to users, nullable — the owner of the deleted repository; set to null when that user is deleted |
| `repository_name` | string | Display name of the deleted repository, kept for admin context |
| `inserted_at` | timestamp | Retirement time |

**Unique constraint**: `(slug)`

The retired-slug record is inserted in the same transaction that deletes the repository. Repository creation treats a retired slug as taken (`422 validation_failed` with `details.slug`, identical to an in-use slug) unless the creator is the retired slug's owner, in which case the creating transaction deletes the retired-slug record and proceeds. Admins can release retired slugs for general reuse from the admin UI; releases are recorded in the audit log.
...
| `size_archive` | bigint | Optional uncompressed payload (cpio archive) size in bytes from the signature header's `PAYLOADSIZE`/`LONGARCHIVESIZE` entry; null when absent |
...
| `header_start` | bigint | Byte offset of the first byte of the main header within the RPM file |
| `header_end` | bigint | Byte offset one past the last byte of the main header within the RPM file |
...
Persisted packages require `name`, `epoch`, `version`, `release`, `arch`, `summary`, `description`, `license`, `size_installed`, `size_package`, `sha256`, `header_start`, and `header_end`. The `url`, `rpm_sourcerpm`, `rpm_group`, `rpm_vendor`, and `rpm_buildhost` fields are nullable when absent or empty after trimming, `build_time` is nullable when the RPM omits the `BUILDTIME` tag, and `size_archive` is nullable when the signature header carries no payload-size entry. `header_start` and `header_end` are always known because the parser has to locate the main header to read any metadata at all. Dependency, file, and changelog fields are stored as empty arrays when the RPM has no entries for that category.
...
| `metadata` | jsonb | Extracted preview metadata (`name`, `epoch`, `version`, `release`, `arch`, `summary`, `description`, `url`, `license`, `rpm_sourcerpm`, `rpm_group`, `rpm_vendor`, `rpm_buildhost`, `size_installed`, `size_archive`, `build_time`, dependencies, files, and changelogs) using the same dependency, file, and changelog shapes as package detail responses |
...
Repo owners and admins can grant other users read access to private repositories. When the invited email address, after normalization, matches an already registered user, a collaborator record is created immediately. When the normalized email does not match a registered user, a pending invitation is created instead and converts to a collaborator record when a matching user account is created. The invited user receives an email notification: registered users get a direct link to the repository, and unregistered invitees get a registration link that converts the pending invitation on signup. When `REGISTRATION_ENABLED = false`, pending invitations to unregistered addresses are still created so they convert automatically once an admin provisions the account, but no email is sent to the unregistered invitee and `notification_sent` stays `false`. The inviting user is shown a UI notice indicating that an admin must create the account before the invitation can be accepted, and API clients read the same fact from the invitation row's `notification_sent` field. `notification_sent` is set to `true` whenever an invitation email is dispatched, at creation or on a refresh. An add request naming an address that already has an unexpired invitation with `notification_sent = false` re-attempts the email instead of returning unchanged — still subject to the same registration-disabled suppression while the address remains unregistered and registration is disabled — so an invitation created while registration was disabled is delivered once registration is enabled.
...
| `expires_at` | timestamp | Expiration time (`INVITATION_EXPIRY_DAYS` after creation; null when expiry is disabled) |
| `notification_sent` | boolean | Whether an invitation email has been dispatched for this invitation (default `false`) |
...
| `action` | string | Dotted event name (e.g., `auth.login`, `auth.login_failed`, `package.upload`, `gpg_key.replace`, `admin.user_delete`, `admin.slug_release`) |
| `target_type` | string | Affected resource type (`repository`, `package`, `user`, `api_key`, `gpg_key`, `collaborator`, `invitation`, `slug`); null when not applicable |
...
- When a user is deleted, the database cascades remove their API keys, session tokens, GPG key, web upload preview rows they initiated, and pending collaborator invitations they sent, removes any collaborator membership rows where they are the collaborator, and deletes any pending collaborator invitations addressed to the deleted user's normalized email so a later re-registration with the same email does not silently re-attach to old invites. The `tmp_path` values of those preview rows are collected before deletion, and cleanup of their temporary files is enqueued through the preview cleanup job, as with repository deletion. Repositories owned by other users on which the deleted user was a collaborator are otherwise unaffected. Retired-slug records from repositories the user previously deleted are retained with `user_id` cleared, so those slugs stay reserved until an admin releases them, and audit events keep their `actor_email` snapshot while `actor_id` is cleared.
...
Repository creation writes the repository row, generates empty `primary.xml.gz`, `filelists.xml.gz`, `other.xml.gz`, `repomd.xml`, and optional `repomd.xml.asc`, and writes the metadata-cache row with `source_revision = 0` in the same database transaction. A creation that reuses the creator's own retired slug deletes the retired-slug record in this same transaction. Newly created empty repos therefore immediately serve valid metadata from the cache. If a repository is created with `gpg_key_fingerprint` set and synchronous `repomd.xml` signing fails for an infrastructure reason, the transaction is rolled back and the caller receives `503 signing_unavailable`; validation failures still return `422 validation_failed`. Repository setting changes that affect generated metadata, such as enabling/disabling metadata signing or changing `gpg_key_fingerprint`, use the same `metadata_revision` increment and regeneration enqueue path.
...
- **Public repodata, `RPM-GPG-KEY`, and `dark-zenith.repo`**: `Cache-Control: public, max-age=0, must-revalidate`, plus `ETag` (the served bytes' lowercase hex SHA-256, quoted) and `Last-Modified` — the metadata-cache row's `updated_at` for repodata, the owner record's `updated_at` for `RPM-GPG-KEY`, and the repository row's `updated_at` for `dark-zenith.repo`. Requests carrying `If-None-Match` or `If-Modified-Since` that match get `304 Not Modified` with an empty body. Revalidation on every fetch keeps a shared cache from widening the checksum-mismatch race described under Metadata Generation & Storage, while still letting `dnf` and the cache skip the transfer when nothing changed.
...
- Every repository-serving response also sends `Vary: Authorization`, because the same path is reachable both anonymously and with Basic Auth credentials.
...
**`SECRET_KEY_BASE` rotation procedure.** Because both `v1` and `v2` derive their AEAD key from `SECRET_KEY_BASE`, rotating that value would otherwise strand every existing `gpg_key_private` row. To rotate safely, the operator sets the new value on `SECRET_KEY_BASE` and provides the prior value on `PREVIOUS_SECRET_KEY_BASE`. Both are read at boot. Encryption always derives its key from `SECRET_KEY_BASE`. Decryption first attempts the current envelope using `SECRET_KEY_BASE`; on AEAD authentication failure it retries with `PREVIOUS_SECRET_KEY_BASE` if configured. When `PREVIOUS_SECRET_KEY_BASE` is set at boot, the application enqueues a unique Oban scan job (`DarkZenith.Jobs.GpgKeyReencryption`) that enqueues one re-encryption job per user row with `gpg_key_private` set. Each per-row job decrypts its row using whichever base succeeds, re-encrypts it under `SECRET_KEY_BASE` as a `v2` envelope, and writes the new ciphertext in a single transaction. Per-row jobs retry up to 20 attempts with exponential backoff; exhausted jobs remain visible in Oban for admin intervention. The scan job runs on each boot while `PREVIOUS_SECRET_KEY_BASE` is set; when it finds no rows that decrypt only with the previous base, it logs completion and the operator can remove `PREVIOUS_SECRET_KEY_BASE` from the deployment. Rotating `SECRET_KEY_BASE` without configuring `PREVIOUS_SECRET_KEY_BASE` immediately strands all existing `gpg_key_private` rows and they fail closed.
...
RPMs that are already signed are re-signed (the existing signature is replaced). This ensures all packages in the repo are signed with a consistent key. Source RPMs are signed and verified through the same flow as binary RPMs. External tools (`rpm`, `rpmsign`, `gpg`) are always invoked directly with argument vectors, never through a shell, so no metadata value is ever interpreted as shell syntax.
...
Unlike `replace_key`, key removal is not blocked by an active key-replacement transition: `DELETE /api/v1/gpg_key` and the `clear_metadata_signing` and `delete_signed_packages` strategies remain available while `previous_gpg_key_public` is set, so revocation stays usable as an escape hatch from a stuck transition. Every successful key removal — the immediate `DELETE` or either non-replacement strategy — also clears `previous_gpg_key_public` and `gpg_key_transition_id` in the same transaction that removes the key. This is safe: the immediate `DELETE` applies only when no repository uses the key, and the strategy paths clear `gpg_key_fingerprint` and enqueue metadata regeneration for every affected repository (bumping `metadata_revision`, so stale signed metadata is never served), leaving no repository whose signatures chain to either key. In-flight re-sign jobs from the abandoned transition observe the `transition_id` mismatch and no-op.
...
Per-user storage is bounded by `MAX_USER_STORAGE_BYTES` (default 50 GiB; `0` disables the quota). A user's usage is the sum of `size_package` across all packages in repositories they own; uploads by an admin to another user's repository count against the owner's quota. The quota is checked when an upload begins (and again at web preview confirmation): if current usage plus the uploaded file size would exceed the quota, the request is rejected with `409 conflict_storage_quota_exceeded` before parsing, signing, or B2 upload. Concurrent uploads may transiently overshoot by at most the size of the in-flight files, and background re-sign jobs update `size_package` without quota checks. Admins can view per-user storage usage in the admin UI.
...
1. **Validate**: Confirm the file is a valid RPM by reading the RPM lead and header. Both binary and source RPMs are accepted; the lead's `type` field (`0` = binary, `1` = source) determines which, and source packages follow the source-specific extraction rules in step 2. While reading, the parser enforces structural bounds: the combined signature and main header regions must not exceed 64 MiB (67 108 864 bytes) and each header must not exceed 65 535 index entries; files exceeding either bound are rejected with `422 validation_failed` before further parsing, so a crafted header cannot exhaust memory.
2. **Extract metadata**: Parse the RPM headers to extract name, version, release, epoch, arch, dependencies, file lists, changelogs, summary, description, license, etc. This is done in Elixir by reading the RPM binary format directly (RPM header structure). Required RPM header metadata is `name`, `version`, `release`, `arch`, `summary`, `description`, `license`, and `size_installed`; `epoch` defaults to `0` when absent. Optional `url`, `rpm_sourcerpm`, `rpm_group`, `rpm_vendor` (`VENDOR` tag), and `rpm_buildhost` (`BUILDHOST` tag) values are stored as `NULL` when absent or empty after trimming; `build_time` is read from the `BUILDTIME` header tag (an unsigned 32-bit Unix timestamp) and stored as `NULL` when absent; `size_archive` is read from the signature header's `PAYLOADSIZE` entry (`LONGARCHIVESIZE` when the payload exceeds the 32-bit range) and stored as `NULL` when neither is present; and dependency, file, and changelog collections default to empty arrays. For source RPMs, `arch` is stored as the literal `src`, matching the repodata convention for source packages; the header `ARCH` tag (which records the build architecture) is ignored and not required, the extracted file list contains the bare source and spec file names from the header (no leading directory), and `rpm_sourcerpm` is `NULL`. Binary RPMs whose header `arch` is `src` are rejected with `422 validation_failed`. Verify that `epoch` is an integer in the unsigned 32-bit range (`0 ≤ epoch ≤ 4 294 967 295`), that `name`, `version`, `release`, and `arch` each match `^[A-Za-z0-9._+~-]+$` and are at most 256 characters long, and that other extracted strings respect the maximum lengths defined in the Packages data-model table (`summary` ≤ 256, `description` ≤ 65 536, `url` ≤ 256, `license` ≤ 256, `rpm_sourcerpm` ≤ 256, `rpm_group` ≤ 256, `rpm_vendor` ≤ 256, `rpm_buildhost` ≤ 256, all measured after trimming). A non-empty `url` must additionally parse as an absolute `http` or `https` URL — it is the only RPM-derived value the web UI renders as a hyperlink, and the scheme restriction keeps `javascript:` and similar URLs out of generated links. Any value that fails these checks is rejected with `422 validation_failed`. All extracted string values — including summaries, descriptions, changelog authors and text, file paths, and dependency names, versions, and releases — must additionally be valid UTF-8 and must not contain characters disallowed by XML 1.0 (control characters other than tab, LF, and CR, and the noncharacters U+FFFE and U+FFFF): these strings are embedded in generated repodata XML, which cannot represent such characters even escaped, so a single RPM carrying them would render the repository's metadata unparsable. RPMs containing such values are rejected with `422 validation_failed`. Extracted collections are also bounded: at most 262 144 file entries, 4 096 changelog entries, and 65 536 entries in each dependency list (requires, provides, conflicts, obsoletes); RPMs exceeding these bounds are rejected with `422 validation_failed`, keeping stored rows and generated repodata bounded. Verify finally that the composed B2 storage key (step 7) is at most 1 024 bytes, the B2 file-name length limit; RPMs whose metadata would produce a longer key are rejected with `422 validation_failed`. Re-sign storage keys differ from upload keys only in their UUID segment, so the same bound covers them. This keeps B2 storage keys constrained to a safe, predictable character set and prevents oversized fields from ballooning generated repodata.
3. **Sign** (if `sign_rpms` enabled): Sign the RPM using the owner's GPG key and `rpmsign` (see GPG Signing section).
4. **Checksum, final size, and header range**: Compute SHA-256, `size_package`, `header_start`, and `header_end` from the final RPM file after any signing step has completed. Signing rewrites the signature header, which moves the main header, so these offsets are always taken from the stored bytes rather than from the pre-signing parse.
...
Repository deletion is a hard delete. When an authorized owner or admin deletes a repository, Dark Zenith reads the current package `storage_path` values and pending web upload preview `tmp_path` values, deletes the repository row and dependent packages, collaborators, invitations, the metadata-cache row, and web upload preview rows, and inserts a retired-slug record for the repository's slug (see Retired Slugs) in one database transaction, and returns `204 No Content` after that transaction commits. B2 object deletion happens after commit through idempotent cleanup jobs for the collected storage paths; temporary preview-file deletion happens through the preview cleanup job. If one or more B2 cleanup jobs fail, the repository remains deleted from PostgreSQL and inaccessible through the web, API, metadata, and package download endpoints while cleanup continues through Oban retries. Previously issued signed B2 URLs for the repository's RPM objects may remain usable until they expire or until the B2 objects are deleted, whichever happens first. Pending or running repository-scoped jobs (metadata regeneration and per-package re-sign jobs) that find their repository row already deleted complete successfully as no-ops.
...
The parser reads the lead, signature, and main header sections — the payload is stored as-is and never decompressed by Dark Zenith. It enforces the structural bounds defined in the upload pipeline (64 MiB combined header regions, 65 535 index entries per header) while reading, so crafted headers cannot exhaust memory. Initial RPM signing relies on the system `rpmsign` and `rpm --checksig` tools instead of custom RPM signature mutation.
...
- **Owner/admin only**: "Manage Collaborators" section to add/remove users who can access a private repo.
  ...
  - Adding a collaborator whose normalized email already has a collaborator row or pending invitation is idempotent; the UI shows the existing collaborator or invitation instead of creating a duplicate, re-attempting the email for an unexpired invitation whose `notification_sent` is still `false`. Expired-but-uncleaned invitations are refreshed with a new expiration and a new invitation email (subject to the registration-disabled email exception under Repository Collaborators). Pending invitations display their expiration time.
...
- **Preview**: The uploaded RPM is size-checked, quota-checked, validated, and parsed using the same validation and metadata extraction rules as the final upload pipeline, and an advisory duplicate check rejects the preview as a duplicate (the web equivalent of `409 conflict_duplicate_package`) when the extracted NEVRA already exists in the repository — the confirmation-time duplicate check and the database unique constraint remain authoritative for concurrent uploads. No package row is created and no B2 object is written during preview. On success, Dark Zenith stores the uploaded file in a temporary path under `RPM_UPLOAD_TMPDIR`, creates a Web Upload Preview record with a 15-minute expiration, and shows the extracted metadata for confirmation.
...
- **Expiration**: Expired previews cannot be confirmed. A preview token that is unknown or already consumed is treated the same as an expired one, as is a preview whose temporary file no longer exists (for example, after an app-server restart when `RPM_UPLOAD_TMPDIR` is on tmpfs). In every such case the UI asks the user to upload the RPM again.
...
- **Retired slugs**: List slugs retired by repository deletion and release individual slugs for general reuse; releases are audited.
...
- `POST /api/v1/repos/:slug/packages`: multipart body with exactly one `rpm` file field. A successful upload returns `201 Created` with the package detail resource. A duplicate NEVRA in the repository returns `409 conflict_duplicate_package`. Oversized uploads return `413 payload_too_large`; uploads that would exceed the repository owner's storage quota return `409 conflict_storage_quota_exceeded`; valid-size files that are not valid RPMs return `422 validation_failed`; insufficient temporary upload workspace returns `503 upload_temp_space_unavailable`; signing infrastructure failures return `503 signing_unavailable`; object-storage failures return `503 storage_unavailable`.
- `POST /api/v1/repos/:slug/collaborators`: JSON body `{"email": "user@example.com"}`. Adding collaborators or invitations is valid only for private repositories; add requests on a public repository are rejected with `422 validation_failed`. Removal, cancellation, and listing remain available regardless of repository visibility, so rows retained across a private-to-public flip can still be managed. The email is normalized to lowercase before lookup. If the email belongs to the repository owner, the request is rejected with `422 validation_failed`. If a collaborator or pending invitation already exists for the normalized email, the request succeeds idempotently with `200 OK` and returns the existing collaborator or invitation instead of creating a duplicate; if that invitation is unexpired with `notification_sent = false`, the request additionally re-attempts the invitation email before returning it, subject to the registration-disabled email exception described under Repository Collaborators. If the existing invitation has expired but has not yet been cleaned up, the add request instead refreshes it — `expires_at` is reset and a new invitation email is sent, following the registration-disabled email exception described under Repository Collaborators — and returns `200 OK` with the refreshed invitation. Newly created collaborators or invitations return `201 Created`.
...
- `PUT /api/v1/gpg_key`: multipart body with `public_key` and `private_key` fields containing ASCII-armored GPG keys. Each key field is capped at 1 048 576 bytes; larger key uploads are rejected with `413 payload_too_large`. The public and private keys must share the same fingerprint, and the private key must be usable for non-interactive signing; key pairs that fail these checks — unparsable key material, mismatched fingerprints, or a private key that fails the test signature (passphrase-protected, expired, or revoked) — are rejected with `422 validation_failed`. The response is `200 OK` with the GPG key resource. If `previous_gpg_key_public` is already set for the user, or if any repository owned by the user has `rpm_signing_state = "signing"`, the request is rejected with `409 conflict_gpg_key_transition_in_progress`.
...
- Package detail resources include every package list field plus `description`, `url`, `license`, `size_installed`, `size_archive`, `build_time`, `rpm_sourcerpm`, `rpm_group`, `rpm_vendor`, `rpm_buildhost`, `requires`, `provides`, `conflicts`, `obsoletes`, `files`, and `changelogs`. `build_time` is an ISO-8601 UTC timestamp or `null`. Package resources never expose the internal `storage_path`, `header_start`, or `header_end`.
...
Whether `dnf`/`librepo` applies these `username=`/`password=` directives when downloading the `gpgkey` URL, rather than only repodata and packages, varies across dnf versions; implementation must verify key import against a real dnf client, and if the directives are not applied, generated private `.repo` files must embed the credential placeholder in the `gpgkey` URL instead (`gpgkey=https://token:<api-key>@<hostname>/repos/:slug/RPM-GPG-KEY`), as the userinfo-style example under Private Repository Authentication does.
...
| `MAIL_FROM_ADDRESS`    | —           | Sender address for outbound notifications (required for the `zepto` and `smtp` adapters) |
...
Unknown aliases cause the application to refuse to boot. When the selected adapter requires credentials or sender configuration, the application also refuses to boot if the required values are missing; with the default `MAIL_ADAPTER=zepto`, both `ZEPTO_API_KEY` and `MAIL_FROM_ADDRESS` are required, and with `MAIL_ADAPTER=smtp`, `MAIL_FROM_ADDRESS` is required alongside the SMTP connection settings. Development and test deployments may set `MAIL_ADAPTER=local` to avoid external mail credentials. Additional Swoosh adapters can be wired in by adding a new alias to this mapping in a future release.
...
RPM files are stored exclusively in Backblaze B2. The app server handles no RPM file traffic — clients are redirected to signed B2 URLs. This means the app itself is lightweight and can run on modest hardware regardless of repository size or download volume. The B2 application key should be scoped to the single bucket and limited to the read, write, and delete capabilities the application uses.
...
- Production deployments enable `force_ssl` (TLS redirect plus `Strict-Transport-Security`) at the Phoenix endpoint even behind the reverse proxy, honoring the terminating proxy's `X-Forwarded-Proto` (`rewrite_on: [:x_forwarded_proto]`) so proxied HTTPS requests are not redirect-looped; session cookies are `secure`, `http_only`, and `SameSite=Lax`; responses set `X-Content-Type-Options: nosniff`; and web UI responses carry a restrictive, LiveView-compatible `Content-Security-Policy`.
...
- **Authentication attempts** (`/api/v1/auth/login`, the web login route, registration, password reset, and the email-confirmation resend route): 10 requests per minute per IP address and 10 requests per minute per normalized (trimmed, lowercased) email address. These buckets apply *in lieu of* the 120/min unauthenticated general bucket — requests to these routes do not also count against the general bucket.
...
For per-IP buckets, IPv6 clients are bucketed by their /64 prefix rather than the full address, since a typical IPv6 allocation makes individual addresses free to mint. Rate-limit state lives in ETS keyed by attacker-influencable values (IP addresses, submitted email addresses), so a periodic sweep, running every 60 seconds, removes bucket entries that have been idle for a full window, bounding memory growth from bucket-key minting.
