...
| `signing_unavailable`          |          `30` | Repository creation with metadata signing; GPG key upload/replacement | A `gpg`/`rpmsign` host fault rarely clears in seconds, and each retry re-runs CPU-heavy key validation. |
