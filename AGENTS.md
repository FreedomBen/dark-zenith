...
for controllers, LiveViews, components, and API endpoints, `priv/repo/migrations/` for
database migrations, `test/` for ExUnit tests, and `assets/` for frontend code. Primary
keys are UUIDs (`--binary-id`).

## Build, Test, and Development Commands

Development and test use a dedicated PostgreSQL 18 podman container on host port 55432
(so the machine's default 5432 stays free for other projects):

- `podman start dark-zenith-pg` — start the existing database container.
- First-time creation: `podman run -d --name dark-zenith-pg -e POSTGRES_PASSWORD=postgres -e POSTGRES_USER=postgres -v dark-zenith-pgdata:/var/lib/postgresql -p 127.0.0.1:55432:5432 docker.io/library/postgres:18`
...
When writing shell scripts, interpolate variables with braces, for example `"${VAR}"`.
If Makefiles are added, default `PREFIX` to `/usr/local` and keep `make help` accurate.
...
`:container`-tagged ones serve the app and the bucket over real HTTP through
`DarkZenith.LiveListeners` and run `deploy/dnf_client_check.sh` in a fresh Fedora 44 container
(via `DarkZenith.DnfClientContainer`) as well as `deploy/live_install_check.sh` as a whole; they
are excluded unless podman, jq, and the image are present.

## Commit & Pull Request Guidelines

Recent history uses concise imperative commit subjects such as `Clarify repository
design contract` and `Resolve design review findings in DESIGN.md`. Follow that style:
describe the change directly, without `feat:` or `bug:` prefixes. Commit bodies should
explain what changed and why when the subject is not enough. Do not add Claude co-author
lines.

Pull requests should include a short summary, test results or an explanation when tests
do not apply, linked issues when relevant, and screenshots for UI changes.

## Agent-Specific Instructions

Do not read `TODO.md` or other TODO files. Do not create or switch branches or push
unless explicitly instructed. Always update documentation when code changes affect
documented behavior.
...
The remainder of this file is the agent guidance generated by `mix phx.new` (Phoenix
1.8) for working in this codebase.
