...
- **Recent Uploads** (owner/admin only): a status list of the repository's retained upload intents, so an uploader who has navigated away from the upload page — or a manager who was not the uploader — can still see how an upload ended. Visibility follows the repository-scoped listing exception in the REST API section; the section is rendered only for the owner or an admin, and it is omitted entirely when the repository has no retained intent rows.
  - Each row shows the original filename, the initiating user (the viewer's own rows are labeled as theirs), whether the upload came from the web or the API, the declared size, a status badge, and the time of the last state change. A `succeeded` row links to the resulting package version page. A `failed` row shows the sanitized `last_error_code` and, when present, the `last_error_detail` reason from Upload Failure Reasons. A `preview_ready` row links to the upload page, where only its initiator can confirm or discard it.
  - Rows are ordered by `inserted_at` descending, then `id` ascending. The section renders the 10 most recent rows plus the total when more exist and does not paginate; the REST listing is the paginated surface for a longer view.
  - The list is bounded by intent retention rather than being a permanent upload history: terminal intents are deleted 24 hours after they finish (see Upload Intents), so the section shows every active upload plus the last day of finished ones. Packages that uploaded successfully remain in the package list indefinitely, which is where an upload older than the retention window is verified.
  - While at least one listed intent is non-terminal, the LiveView refreshes the section on a five-second server-side timer and stops once every listed row is terminal. That refresh is an internal message rather than a user-initiated event, so it consumes no rate-limit bucket (see Rate Limiting).
  - Rows the viewer initiated that are still cancelable offer the same cancellation action as the upload page, under the unchanged initiator-only rule; rows another user initiated are read-only.
...
- `GET /api/v1/repos/:slug/package-uploads`: lists the repository's retained upload intents in the standard paginated envelope, ordered by `inserted_at` descending then `id` ascending. It is authorized as a repository-scoped read for the owner or an admin under the listing exception in the REST API section, so it returns rows from every initiator rather than only the requester's; API-key requests require `package:upload`, keeping the whole `package-uploads` family under one scope. It accepts an optional `status` filter whose value is a comma-separated subset of the documented intent statuses (`awaiting_upload`, `queued`, `processing`, `preview_ready`, `succeeded`, `failed`, `expired`, `canceled`); an unknown status, a blank entry, or an entry repeated within the list is rejected with `422 validation_failed`, and repeating the `status` key itself is already rejected by the strict query-parsing rule above. Each row carries `id`, `repository_id`, `user_id`, `user_email`, `package_id`, `mode`, `status`, `original_filename`, `declared_size`, `upload_generation`, `attempts`, `last_error_code`, `last_error_detail`, `expires_at`, `completed_at`, `inserted_at`, and `updated_at`; `user_email` is the initiator's current normalized address. A listing row never carries an `upload` capability object, `staging_path`, `staging_version_id`, `lease_token`, or `preview_metadata`, so preview metadata is still read only through the single-intent endpoint by the user who initiated that intent. Because terminal rows are deleted 24 hours after they finish, this endpoint is a recent-activity index rather than a durable upload history, and a client that needs a permanent record keeps its own or reads the resulting packages.
