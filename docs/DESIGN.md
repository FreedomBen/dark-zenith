...
| `last_error_detail` | string | Optional sanitized reason refining `last_error_code`, drawn from the closed Upload Failure Reasons vocabulary; null when the failure has no more specific classification, and cleared with `last_error_code` |
...
Upload intents are operational state and are deleted 24 hours after they finish. A package upload record is the durable counterpart: one append-mostly row per upload attempt that outlives its intent, its repository, and its initiator's account, so "what was uploaded here, by whom, and how did it end" stays answerable indefinitely.

| Field | Type | Description |
|---|---|---|
| `id` | UUID | Primary key |
| `repository_id` | UUID | Target repository as a UUID snapshot rather than a foreign key, so a repository hard delete does not remove the record |
| `repository_slug` | string | Snapshot of the repository slug at upload time, so a record stays readable after the repository is gone |
| `user_id` | UUID | Nullable FK to users (`ON DELETE SET NULL`) — the owner/admin who initiated the upload |
| `user_email` | string | Snapshot of the initiator's email at upload time, so attribution survives account deletion |
| `intent_id` | UUID | Snapshot of the upload intent this record describes, not a foreign key; unique |
| `package_id` | UUID | Snapshot of the intent's preallocated package id; identifies the resulting package row on success |
| `mode` | string | `api` or `web_preview`, copied from the intent |
| `original_filename` | string | Client-supplied filename, copied from the intent under its validation rules |
| `declared_size` | bigint | Client-declared source bytes at intent creation |
| `final_size` | bigint | Stored size of the resulting package after optional signing; null unless `outcome = "succeeded"` |
| `outcome` | string | `in_flight`, `succeeded`, `failed`, `expired`, or `canceled` |
| `error_code` | string | Sanitized terminal error code; null unless `outcome = "failed"` |
| `error_detail` | string | Optional sanitized Upload Failure Reasons value refining `error_code`; null unless `outcome = "failed"`, and null even then when the failure has no more specific classification |
...
| `finished_at` | timestamp | Terminal transition time; null while `in_flight`. For a record finalized by reconciliation rather than by its own upload's terminal transition, this is the sweep's timestamp, so it is an upper bound on the true end rather than the end itself |
...
**Unique constraints**: `(intent_id)`. The repository listing is served by an index on `(repository_id, started_at DESC, id)`; `(user_id)` and `(package_id)` are indexed for account-scoped and package-scoped lookups. A partial index on `(repository_id)` where `outcome = 'in_flight'` serves the repository's in-flight count, which the Upload History section reads on every render to decide whether to keep refreshing; in-flight rows are a small and self-limiting subset, so the partial index stays cheap as history accumulates.
...
Records deliberately survive what they describe. Repository deletion and user deletion both delete upload intents, so each of those transactions finalizes any `in_flight` record for the intents it removes as `canceled` before committing, and neither deletes the records themselves. The hourly terminal-intent cleanup applies the same rule as a safety net, through a second query in the same job: an anti-join from `in_flight` records against upload intents finalizes any record whose intent row no longer exists as `canceled`, so no record stays `in_flight` after its intent is gone. That path exists for an intent that disappeared by some route the rules above did not anticipate, so it cannot know when the upload actually ended and writes the sweep's own timestamp to `finished_at`; a record finalized this way therefore reports a duration no longer than one sweep interval beyond the truth. Between an intent's disappearance and the next sweep, a record can be `in_flight` with no backing intent for up to an hour, which both surfaces render as described under Upload History. Because a record holds no capability, no staging key, no B2 version, and no parsed package metadata, retaining it indefinitely creates no cleanup obligation against B2 and no path back to object storage.

A record duplicates a small and deliberately chosen part of what the audit log holds. The audit log is the security record, ordered by event and read by admins; upload records are the product record, ordered by upload and read by repository managers. Both are written in the same transaction as the action they describe, and the upload audit events carry `upload_record_id`, so a row in either can be resolved to its counterpart.

Retention has a privacy consequence worth stating plainly: a filename, a size, and an initiator's email address persist after the repository, the package, and even the initiating account are gone. This matches the existing treatment of audit events, which keep `actor_email` after `actor_id` is cleared, and it is why records carry no parsed package metadata, no file lists, and no storage identifiers — the durable record is deliberately the smallest set of fields that answers the question it exists to answer.
...
| Scope | Permits |
|---|---|
| `repo:read` | Read access to private repositories the user can access (as owner, collaborator, or admin), including listing collaborators, pending invitations, and package upload records for a repository the user owns or administers |
...
Audited actions: successful and failed logins (web and API), password changes and resets, account email changes, account registration and admin account creation, API key creation and revocation, GPG key upload, generation, replacement, removal, and revocation strategies, repository creation, settings changes, and deletion, upload-intent creation/refresh/cancellation and terminal package-upload outcomes, package deletions, collaborator additions and removals, invitation creation, cancellation, conversion, and expiry refresh, signing-transition item and phase resets and transition cancellation, admin-flag changes, recovery promotion, user deletion, and slug-reservation releases. Audit metadata records intent IDs, generation numbers, sizes, and stable result codes but never presigned URLs, B2 version IDs, or GPG key material. Every `package.upload*` event is self-sufficient: alongside its action-specific fields it carries `repository_id`, `repository_slug`, `intent_id`, `upload_record_id`, and `original_filename`, so a terminal failure identifies its repository and file without a join back to the creation event, and so any upload event resolves to its Package Upload Record and back. The success event keeps its `nevra` and `result` fields and targets the resulting package; failure, expiry, and cancellation events target the upload intent. Failed-login volume is bounded by the authentication-attempt rate limits. Admins browse the audit log in the admin UI.
...
`last_error_code` is the coarse, contract-stable outcome; many distinct rejections share
`validation_failed`, which alone does not tell an uploader what to fix. A deterministic failure
therefore also records an optional `last_error_detail` drawn from a closed vocabulary that
classifies the submitted file or the pipeline stage that rejected it. The vocabulary is a
published contract: names are stable across releases, and a reason outside it is never surfaced —
the detail is simply null, so a new internal error term cannot leak by default. A reason never
contains `rpmkeys`/`rpmsign`/`gpg` output, stderr, filesystem paths, B2 keys or version IDs, or
any other storage detail; it is a classification, not a diagnostic dump. Because every reason
describes the uploader's own submitted bytes or their own repository's configuration, it is
disclosed to any identity permitted to read the intent and, through the repository's upload
listing, to the repository's owner and admins (see Package Upload Records); it is recorded in
the audit log alongside the code.

Structural rejections from step 1 use the reason names `bad_lead_magic`, `bad_header_magic`,
`unsupported_format`, `truncated`, `header_too_large`, `too_many_entries`, `empty_header`,
`duplicate_tag`, `unknown_entry_type`, `invalid_count`, `misaligned_entry`, `entry_out_of_bounds`,
`overlapping_entries`, `missing_region`, `invalid_region`, `unsorted_tags`, `nonzero_padding`,
`weak_digests`, `v6_bad_encoding`, `v6_missing_mandatory_tag`, `v6_reserved_not_zero`,
`v6_signature_forbidden_tag`, `v6_signature_missing_reserved`, and `v6_weak_file_digest`.
Semantic rejections from step 3 use `missing_required_field`, `malformed_header_value`,
`malformed_i18n`, `invalid_nevra`, `invalid_string`, `invalid_url`, `string_too_long`,
`malformed_dependency`, `malformed_files`, `malformed_changelogs`, `malformed_sourcenevr`,
`missing_sourcenevr`, `too_many_dependencies`, `too_many_files`, and `too_many_changelogs`.

The remaining deterministic pipeline stages classify themselves:

| Reason | Stage | Meaning |
| --- | --- | --- |
| `integrity_check_failed` | 2 | The verifier reported a `BAD` digest or signature, or reported no digest at all |
| `preview_metadata_mismatch` | 3 | Confirmation re-extracted metadata that differs from the stored `preview_metadata` |
| `signing_key_not_configured` | 4 | `sign_rpms` is enabled but the repository owner has no signing key configured |
| `signed_output_verification_failed` | 4 | The signed output failed the required expected-key verification |
| `storage_key_too_long` | 9 | The composed final B2 key exceeds 1 024 UTF-8 bytes |
...
- **Upload History** (owner/admin only): one list covering both questions a manager asks about uploads — what is happening right now, and what has ever happened. It reads Package Upload Records, so it is complete for the life of the repository rather than bounded by the 24-hour intent retention, and it left-joins the live upload intent so in-flight rows report real-time progress. Visibility follows the repository-scoped listing exception in the REST API section; the section renders only for the owner or an admin, and is omitted entirely when the repository has no records.
  - Each row shows the original filename, the initiating user (the viewer's own rows are labeled as theirs), whether the upload came from the web or the API, the size, a status badge, and the start and finish times. A succeeded row shows the resulting NEVRA and links to its package version page — which stays correct after the package itself is deleted only insofar as the link then 404s, since the record outlives the package. A failed row shows the sanitized `error_code` and, when present, the `error_detail` reason from Upload Failure Reasons.
  - An `in_flight` row whose intent still exists shows that intent's live status (`awaiting_upload`, `queued`, `processing`, or `preview_ready`) instead of the bare `in_flight` outcome; a `preview_ready` row links to the upload page, where only its initiator can confirm or discard it. A terminal row shows its recorded outcome permanently, whether or not its intent is still within the 24-hour retention window — the outcome was recorded in the same transaction as the intent's terminal transition, so there is nothing fresher to show.
  - An `in_flight` row whose intent is already gone is awaiting the reconciliation sweep described under Package Upload Records. It renders as an unknown status rather than as progress, offers no cancellation, and resolves itself to `canceled` within the hour; it is the only row whose displayed status is not the truth about a live upload, and it should not be mistaken for one that is still running.
  - Rows are ordered by `started_at` descending, then `id` ascending, and paginate at 25 per page with the page in the URL. In-flight rows sort first only by virtue of being the most recent; the list is not partitioned, because a pinned duplicate of a row that also appears in the page would make the pagination counts incoherent.
  - The section accepts the same `outcome` filter as the REST listing, held in the URL beside the page as `?outcome=...` and `?page=...` — the REST listing's own parameter names — so a filtered view is reloadable and shareable like the rest of the UI. Its header states the repository's current in-flight count and links to the list filtered to `in_flight`, which is what makes a stuck upload reachable once newer uploads have pushed it off the first page.
  - The refresh condition is that repository-wide in-flight count, not the contents of the current page: while it is nonzero the LiveView refreshes the section on a five-second server-side timer, and it stops once the count reaches zero. Scoping the timer to the visible page would stop refreshing exactly the upload this section exists to surface, since a stuck upload is by definition an old one. The count is a single indexed count on the record's outcome, and the refresh is an internal message rather than a user-initiated event, so it consumes no rate-limit bucket (see Rate Limiting). An awaiting-reconciliation row counts toward it and so can hold the timer open with nothing actually progressing, which is accepted rather than special-cased: the sweep closes it within the hour, and excluding those rows would mean re-deciding on every tick which `in_flight` rows are real.
  - Rows the viewer initiated whose intent is still cancelable offer the same cancellation action as the upload page, under the unchanged initiator-only rule; every other row is read-only.
...
Every package-upload intent endpoint addressed by intent id — `refresh`, `complete`, the single-intent status read, and cancellation — additionally requires the same authenticated user who created the intent; API-key requests require `package:upload`. A second admin cannot take over another user's staged capability: an intent whose initiating `user_id` is not the authenticated user is treated as nonexistent and answered with `404 not_found`, exactly like an id scoped to a different repository. Repository deletion remains able to cancel all intents as a system-side consequence of deleting their target.

The repository-scoped upload listing — `GET /api/v1/repos/:slug/package-uploads` and the equivalent Upload History section on the repository detail page — is the one deliberate exception to that rule. It is authorized as a repository-scoped read for the owner or an admin and returns every Package Upload Record for that repository regardless of initiator, along with the live status of any intent still backing one. The exception is confined to status visibility and is deliberately narrow in three ways. A listing conveys no capability: records store none of the upload machinery, and a listing never returns a presigned `upload` object, `staging_path`, `staging_version_id`, `lease_token`, or `preview_metadata`. It grants no action over another user's intent, because refresh, completion, confirmation, and cancellation remain initiator-only under the rule above. And what it exposes is bounded and defensible field by field. Filename, size, mode, status, timestamps, and the sanitized failure code and reason are already within a repository manager's reach once an upload succeeds, since the resulting package, its size, and its metadata become repository content the manager can read and delete. The initiator's email is the one field that is genuinely new — package resources carry no uploader identity — and it is disclosed deliberately rather than incidentally. Because an upload can only be initiated by the repository's owner or an admin, the initiator set is exactly that pair, so the only cross-user disclosure this creates is an admin's address to the owner of a repository that admin wrote to. Attribution is the point: an administrator acting on someone else's repository should be answerable to its owner for what they put there. Nothing weaker would let an owner tell their own uploads from an operator's. The alternative leaves a repository owner unable to observe that their own repository has a stuck, failing, or quota-consuming upload in flight that another manager started. Authorization tests must cover this boundary for anonymous, unrelated-user, collaborator, non-initiating owner, non-initiating admin, and initiating-user requesters, and must assert that no capability, staging, lease, preview, or `user_id` field appears in any listing row on either surface.
...
GET    /api/v1/repos/:slug/package-uploads                  # List the repository's package upload records (owner/admin only; API keys need `repo:read`)
...
- `GET /api/v1/repos/:slug/package-uploads`: lists the repository's Package Upload Records in the standard paginated envelope, ordered by `started_at` descending then `id` ascending. This collection endpoint lists uploads, while the id-addressed endpoints beneath it act on the live upload intent; each row carries the `intent_id` that addresses them, null once that intent has been cleaned up. It is authorized as a repository-scoped read for the owner or an admin under the listing exception in the REST API section, so it returns rows from every initiator rather than only the requester's; API-key requests require `repo:read`, the same scope that governs the analogous owner/admin-only collaborator listing. Scope follows the verb rather than the URL family: this collection is a read, while the id-addressed endpoints beneath it act on a live capability and keep `package:upload`. A monitoring client can therefore watch a repository's upload health with a read-only key, and an uploader holding only `package:upload` still polls its own upload through the single-intent endpoint. It accepts an optional `outcome` filter whose value is a comma-separated subset of `in_flight`, `succeeded`, `failed`, `expired`, and `canceled`, with each entry matched exactly, without trimming or case folding; an unknown outcome, a blank entry, or an entry repeated within the list is rejected with `422 validation_failed`, and repeating the `outcome` key itself is already rejected by the strict query-parsing rule above. Each row carries `id`, `repository_id`, `repository_slug`, `user_email`, `intent_id`, `package_id`, `mode`, `original_filename`, `declared_size`, `final_size`, `outcome`, `live_status`, `error_code`, `error_detail`, `nevra`, `started_at`, and `finished_at`. The record's `user_id` column is never serialized: there is no user resource in the REST API for it to address, so it would serve only as a handle for correlating one person's activity across repositories. `user_email` is the snapshot taken at upload time and may differ from the initiator's current address, so it identifies who uploaded rather than where to reach them. `live_status` refines `in_flight` and nothing else: it is the backing intent's current status — one of `awaiting_upload`, `queued`, `processing`, or `preview_ready` — while `outcome` is `in_flight` and that intent row still exists, and `null` in every other case. It is `null` for a terminal record even during the 24 hours its intent survives, because the record's `outcome` is written in the same transaction as that terminal transition and can never disagree with it, so a terminal `live_status` would only restate `outcome` and then vanish when the intent was cleaned up. It is also `null` for an `in_flight` record whose intent is already gone, which is the reconciliation window described under Package Upload Records. So a non-null `live_status` implies `outcome = "in_flight"`, and `outcome` is always the durable answer. A listing row never carries an `upload` capability object, `staging_path`, `staging_version_id`, `lease_token`, or `preview_metadata`; preview metadata is still read only through the single-intent endpoint, and only by the user who initiated that intent. Because records outlive their intents, their packages, and their repository, this endpoint is a durable upload history rather than a recent-activity index.
...
- Upload-intent resources have shape `{"id": "<uuid>", "repository_id": "<uuid>", "package_id": "<uuid>", "mode": "api", "status": "processing", "original_filename": "nginx.rpm", "declared_size": "623104", "attempts": 1, "expires_at": null, "error": null, "package": null, "completed_at": null, "inserted_at": "...", "updated_at": "..."}`. `status` is one of the Upload Intents states, although API-mode resources never enter `preview_ready`. `expires_at` is non-null only while awaiting a direct transfer or web confirmation. `error` is null except on `failed`, where it has `{"code": "signing_unavailable", "message": "RPM signing is temporarily unavailable"}`. When the intent carries a `last_error_detail`, the object also has `"details": {"reason": "malformed_header_value", "message": "A header tag has an unexpected physical type."}` — a closed-vocabulary classification of the submitted file drawn from Upload Failure Reasons, never tool output, stderr, filesystem paths, or storage detail. The `code` stays within the documented error vocabulary so clients may keep matching on it. `package` is the package detail resource on `succeeded`; it becomes null if that package is later deleted, while the immutable `package_id` remains available for correlation. The internal reservation, staging key/version, and lease token are never exposed; the status resource also omits the upload generation, while create and refresh responses include the current generation only inside the ephemeral `upload` capability object.
- Upload record rows from `GET /api/v1/repos/:slug/package-uploads` have shape `{"id": "<uuid>", "repository_id": "<uuid>", "repository_slug": "stable", "user_email": "owner@example.com", "intent_id": "<uuid>", "package_id": "<uuid>", "mode": "api", "original_filename": "nginx.rpm", "declared_size": "623104", "final_size": "623104", "outcome": "succeeded", "live_status": null, "error_code": null, "error_detail": null, "nevra": "nginx-0:1.24.0-2.fc39.x86_64", "started_at": "...", "finished_at": "..."}`. `declared_size` and `final_size` are decimal strings under the `bigint` rule. `final_size` and `nevra` are null unless `outcome` is `succeeded`, `error_code` and `error_detail` are null unless it is `failed`, `finished_at` is null while it is `in_flight`, and `live_status` is non-null only for an `in_flight` row whose intent still exists, as defined in the endpoint's contract bullet above.
