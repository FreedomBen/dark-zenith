...
disclosed to any identity permitted to read the intent, and it is recorded in the audit log
alongside the code.
...
- **Upload History** (owner/admin only): one list covering both questions a manager asks about uploads — what is happening right now, and what has ever happened. It reads Package Upload Records, so it is complete for the life of the repository rather than bounded by the 24-hour intent retention, and it left-joins the live upload intent so in-flight rows report real-time progress. Visibility follows the repository-scoped listing exception in the REST API section; the section renders only for the owner or an admin, and is omitted entirely when the repository has no records.
  ...
  - The section accepts the same `outcome` filter as the REST listing, held in the URL beside the page so a filtered view is reloadable and shareable like the rest of the UI. Its header states the repository's current in-flight count and links to the list filtered to `in_flight`, which is what makes a stuck upload reachable once newer uploads have pushed it off the first page.
